...
HexDDD provides building blocks for implementing DDD patterns in Ruby applications. It enforces clean architecture by separating domain logic from infrastructure concerns.
...
```ruby
# Aggregate Root
class Order < HexDDD::Domain::AggregateRoot
  def initialize(id:)
    super(id: id)
    @items = []
  end
...
# Domain Event
class ItemAdded < HexDDD::Domain::DomainEvent
...
class OrderRepository < HexDDD::Ports::SecondaryPort
...
```ruby
# Command
class CreateOrderCommand < HexDDD::Application::Command
  attribute :customer_id, Types::String
  attribute :items, Types::Array
end

# Application Service
class CreateOrderService < HexDDD::Application::Service
